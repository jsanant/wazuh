    ...
    if (S_ISREG(statbuf.st_mode) || S_ISLNK(statbuf.st_mode))
    {
        ...
        if (s_node = (syscheck_node *) OSHash_Get_ex(syscheck.fp, file_name), !s_node) {
            ...
            snprintf(alert_msg, OS_MAXSTR, "%c%c%c%c%c%c%c%c%c%c%c%s:%s:%s::%s:%s:%s:%s:%s:%s:%s:%u",
                    ...
                    user ? user : "",
                    opts & CHECK_GROUP ? get_group(islink_win(file_name) ? statbuf_lnk.st_gid : statbuf.st_gid) : "",
                    str_mtime,
                    str_inode,
                    opts & CHECK_SHA256SUM ? sf256_sum : "",
                    opts & CHECK_ATTRS ? w_get_file_attrs(islink_win(file_name) ? real_path : file_name) : 0);
mdebug2("First CHECKS: %s", file_name);
#else
            if (opts & CHECK_SIZE) {
                if (opts & CHECK_FOLLOW) {
                    if (S_ISLNK(statbuf.st_mode)) {
            ...
            os_free(real_path);
            os_free(user);
mdebug2("Second CHECKS:%s", file_name);
#else
            if (opts & CHECK_SIZE) {
                if (opts & CHECK_FOLLOW) {
                    if (S_ISLNK(statbuf.st_mode)) {
...
//#ifndef WIN32
    ...
    case 2:
        ...
        return 0;
    case 1:
    // TODO dir_name is not the propper variable in WIN32
...
mdebug2("After syscheck entry %s", real_path);
        /* Check for real time flag */
        if (opts & CHECK_REALTIME || opts & CHECK_WHODATA) {
#ifdef INOTIFY_ENABLED
...
char *real_path_win(const char *file_name, char *real_path) {
    ...
    if (rdata->ReparseTag == IO_REPARSE_TAG_SYMLINK) {
        ...
        WCHAR *szPrintName;
        szPrintName = (WCHAR *) malloc(slen+1);
        ...
        wcstombs(aux, szPrintName, plen+1);
        ...
        if(is_relative_path(aux)) {
            ...
            wcstombs(real_path, szPrintName, plen+1);
        }
